...
public:
	...
	bool addConnectionMutation(int& innovationNumber) {
		...
		for (const auto& [id1, type1] : nodes) {
			if (type1 == ENodeType::OUTPUT) continue;  // Changed from string comparison
			
			for (const auto& [id2, type2] : nodes) {
				if (type2 == ENodeType::INPUT) continue;  // Changed from string comparison
				if (id1 == id2) continue;
		...
		for (const auto& [id, type] : nodes) {
			if (type != ENodeType::INPUT && isValidConnection(-1, id)) {  // Changed from string comparison
    ...
    bool addNodeMutation(int& innovationNumber) {
        ...
        // Add new node
        addNode(newNodeId, ENodeType::HIDDEN);  // Changed from string to enum
...
private:
	...
	Genome crossover(const Genome& parent1, const Genome& parent2) {
		try {
			...
			for (const auto& [nodeId, ENodeType] : parent1.getNodes()) {
				child.addNode(nodeId, ENodeType);  // Now accepts ENodeType directly
			}
			for (const auto& [nodeId, ENodeType] : parent2.getNodes()) {
