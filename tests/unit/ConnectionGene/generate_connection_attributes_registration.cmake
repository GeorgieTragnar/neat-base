function(generate_connection_attributes_registration SOURCE_FILE OUTPUT_FILE)
    # Read the source file
    file(READ ${SOURCE_FILE} FILE_CONTENT)
    
    # Extract ConnectionGeneAttributes struct definition
    string(REGEX MATCH "struct[ \t\n]+ConnectionGeneAttributes[ \t\n]*{([^}]*)}" 
           STRUCT_MATCH "${FILE_CONTENT}")
    
    if(NOT STRUCT_MATCH)
        message(FATAL_ERROR "Could not find ConnectionGeneAttributes struct in ${SOURCE_FILE}")
    endif()
    ...
    foreach(LINE ${STRUCT_LINES})
        ...
        if(MEMBER_MATCH)
            ...
            string(APPEND REGISTRATION_CODE 
                   "    register_field(create_field_descriptor(\"${MEMBER_NAME}\", &ConnectionGeneAttributes::${MEMBER_NAME}));\n")
        endif()
    endforeach()
    ...
    message(STATUS "Generated field registration for ConnectionGeneAttributes in ${OUTPUT_FILE}")
endfunction()